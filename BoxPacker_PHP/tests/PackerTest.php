...
function remove_drone($dist, $all_drone){
    ...
    foreach ($all_drone as $drone) {
        if ($drone->getFlybility() > $dist){
            echo "Look here ". $drone->getFlybility(). " ". $dist;
...
class PackerTest extends TestCase
{
    ...
    {
        ob_end_flush();
        for ($gamma = 0.99; $gamma > 0.5; $gamma = $gamma-0.01) {
            echo $gamma;
            echo "\n";
    //        $gamma = 0.7;
    //        $dist = (1 - $gamma) * 51.677;
            ...
            $cef = (1 + (1 - $gamma) / 2);
            $all_data = array(
                array(33.186 * $cef, 1, 2, 0, 1, "Hospital HIMA", 1), array(23.717 * $cef, 0, 1, 1, 0, "Hospital PS", 1), array(12.896 * $cef, 1, 2, 1, 2, "Children’s Hospital", 1), array(51.677 * $gamma, 3, 1, 0, 0, "Hospital PA", 1),
            ...
            for ($x = 12; $x < 60; $x++) {
                ...
                if (count($packedBoxes1) > 1 || count($packedBoxes2) > 2) {
                    ...
                    $myfile = fopen("sensitivity.txt", "a");
                    fwrite($myfile, "gamma is " . (string)$gamma . "   " );
                    fwrite($myfile, (string)($x-1));
                    fwrite($myfile, "\n");
                    fclose($myfile);
