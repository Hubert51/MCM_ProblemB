...
//        print 123;
        ...
        $all_data = array(
            array(33.186,0,2,0,1,"Hospital HIMA",1), array(23.717,1,1,1,0, "Hospital PS",1), array(12.896,2,2,1,2,"Children’s Hospital",1), array(51.677,4,1,0,0,"Hospital PA",1),
            array(16.032,0,2,0,1, "Hospital HIMA",2), array(14.943,0,1,1,0, "Hospital PS",2), array(21.205,0,2,1,2, "Children’s Hospital",2), array(36.474,4,1,0,1, "CMC",2),
            array(16.717,4,1,0,1, "Hospital HIMA",3), array(10.452,3,1,1,0, "Hospital PS",3), array(9.890,2,2,1,2, "Children’s Hospital",3)
        );
...
//        $packedBoxes2 ;
        ...
        for($x =1; $x < 60; $x++) {
            ...
            $container3 = new Packer();
            $container1->addBox($c1);
            $container2->addBox($c1);
            $container3->addBox($c1);
            ...
            foreach ( $all_data as $d ){
                echo "line".$d[6]." to ".$d[5].". Distance is ".$d[0]."\n";
...
//                        echo $boxType->getReference();
                ...
                elseif ( $d[6] == 3 ){
                    ...
                    foreach ($result as $packedBox) {
                        $boxType = $packedBox->getBox(); // your own box object, in this case TestBox
//                        echo $boxType->getReference();
                        $container3->addItem( new TestItem($boxType->getReference(), $boxType->getOuterWidth(), $boxType->getOuterLength(), $boxType->getOuterDepth(), 0, false));
                        if ( $boxType->getReference()=="F" and $flag){
                            $container3->addItem($drone_b);
                            $flag = false;
                        }
                    }
                }
...
//            echo "this is the most important";
            if (count($packedBoxes1) > 1 || count($packedBoxes2) > 1 || count($packedBoxes3) > 1) {
//                echo "this is final n=> ".$x;
                echo count($packedBoxes1) . " " . count($packedBoxes2)  . " " . count($packedBoxes3) ;
                break;
            }

            calculate_drone_number($packedBoxes1, 1);
            calculate_drone_number($packedBoxes2, 2);
            calculate_drone_number($packedBoxes3, 3);
